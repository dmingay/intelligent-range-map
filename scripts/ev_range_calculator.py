...
# Valhalla routing engine (self-hosted via Docker)
VALHALLA_URL = os.getenv("VALHALLA_URL", "http://host.docker.internal:8002")

# Default position (Staines/Surrey) — used when no GPS available
DEFAULT_LAT = float(os.getenv("DEFAULT_LAT", "51.3656"))
DEFAULT_LON = float(os.getenv("DEFAULT_LON", "-0.4139"))

# ─── Polestar 4 Vehicle Parameters ───────────────────────────────────────────

BATTERY_CAPACITY_KWH = 100.0    # Polestar 4 Long Range usable capacity
VEHICLE_MASS_KG = 2435.0        # Kerb weight
DRAG_COEFFICIENT = 0.28         # Cd
FRONTAL_AREA_M2 = 2.62          # Approximate frontal area
ROLLING_RESISTANCE = 0.009      # Tyre rolling resistance coefficient
AIR_DENSITY_KG_M3 = 1.225       # Sea level standard
DRIVETRAIN_EFFICIENCY = 0.90    # Motor + inverter + transmission
REGEN_EFFICIENCY = 0.65         # Regenerative braking recovery
AUX_POWER_KW = 0.4              # Base auxiliary load (lights, electronics)
RESERVE_SOC_PCT = 5.0           # Don't plan to use last 5%

# HVAC power draw estimates (kW) based on temperature
HVAC_POWER_MAP = {
    # (temp_c_lower, temp_c_upper): hvac_kw
    (-30, -10): 5.0,
    (-10, 0): 4.0,
    (0, 5): 3.0,
    (5, 12): 1.5,
    (12, 22): 0.3,   # Minimal — fan only
    (22, 28): 1.5,
    (28, 35): 3.0,
    (35, 50): 4.5,
}

# Energy band definitions (fraction of usable energy)
ENERGY_BANDS = [
...
def get_vehicle_state() -> dict:
    """Read all relevant vehicle states from HA."""
    ...
    return {
        "soc": soc,
        "soh": soh,
        "oem_range_km": oem_range,
        "odometer_km": odometer,
        "charging_status": charging,
    }


def get_vehicle_position() -> tuple[float, float]:
    """Get vehicle GPS. Falls back to default position."""
...
def get_weather(lat: float, lon: float) -> dict:
    ...
    if not OWM_API_KEY:
        log.warning("No OWM API key — using defaults")
        return default
...
def estimate_hvac_power(temp_c: float) -> float:
    """Estimate HVAC power draw based on temperature."""
...
def compute_energy_consumption_kwh_per_km(
    speed_kmh: float = 60.0,
    temp_c: float = 15.0,
    wind_ms: float = 0.0,
    elevation_gain_m_per_km: float = 0.0,
) -> float:
    """
    Physics-based energy consumption model for Polestar 4.
    Returns kWh per km at given conditions.
    """
    v_ms = speed_kmh / 3.6

    # Aerodynamic drag force (headwind increases effective speed)
    ...
    # Gravity (elevation gain)
    f_grade = VEHICLE_MASS_KG * 9.81 * (elevation_gain_m_per_km / 1000.0)

    # Net traction force
    f_total = f_aero + f_roll + max(f_grade, 0)

    # Regen recovery for downhill
    regen_recovery = 0.0
    if f_grade < 0:
        regen_recovery = abs(f_grade) * v_ms * REGEN_EFFICIENCY / 1000.0
    ...
    # Total power draw
    p_total = p_elec + p_hvac + AUX_POWER_KW - regen_recovery
    p_total = max(p_total, 0.5)

    # Energy per km
    kwh_per_km = p_total / speed_kmh
    return kwh_per_km
...
def calculate_range_km(
    ...
    """Calculate effective range in km for a given energy fraction."""
    usable_soc = max(soc - RESERVE_SOC_PCT, 0)
    usable_kwh = BATTERY_CAPACITY_KWH * (soh / 100.0) * (usable_soc / 100.0) * energy_fraction

    consumption_city = compute_energy_consumption_kwh_per_km(35, temp_c, wind_ms, 2.0)
    consumption_suburban = compute_energy_consumption_kwh_per_km(60, temp_c, wind_ms, 5.0)
    consumption_motorway = compute_energy_consumption_kwh_per_km(100, temp_c, wind_ms, 3.0)

    avg_consumption = (
        0.40 * consumption_city
        + 0.40 * consumption_suburban
        + 0.20 * consumption_motorway
    )

    range_km = usable_kwh / avg_consumption if avg_consumption > 0 else 0
    return max(range_km, 0)

# ─── Valhalla Isodistance API ─────────────────────────────────────────────────

def fetch_valhalla_isodistance(lat: float, lon: float, distances_km: list[float]) -> list[dict | None]:
    """
    Fetch isodistance polygons from self-hosted Valhalla in a single request.
    Returns a list of GeoJSON geometries (one per distance), or None for failures.
    Distances are in km. Valhalla returns proper road-network polygons.
    """
    # Build contours — Valhalla accepts multiple in one request
    ...
    log.info(f"Valhalla isodistance request: {len(contours)} contours, "
             f"distances={[f'{d:.0f}km' for d in distances_km]}")
    ...
    try:
        ...
        # Valhalla returns features ordered by contour (largest first)
        # Each feature has properties.contour with the distance value
        # Map them back to our requested distances
        geom_by_distance = {}
        for f in features:
            ...
            if contour_val is not None and geom:
                ...
                log.info(f"  → Contour {contour_val}km: {geom['type']} with "
                         f"{len(geom.get('coordinates', [[]]))} ring(s)")

        # Return in same order as requested distances
        results = []
        for d in distances_km:
            results.append(geom_by_distance.get(d))
        return results

    except requests.exceptions.ConnectionError:
        log.error(f"Cannot connect to Valhalla at {VALHALLA_URL} — is the container running?")
        return [None] * len(distances_km)
    except requests.exceptions.HTTPError as e:
        log.error(f"Valhalla HTTP error: {e} — Response: {e.response.text[:300] if e.response else 'N/A'}")
        return [None] * len(distances_km)
    except Exception as e:
...
def build_geojson(lat: float, lon: float, bands: list[dict]) -> dict:
    """
    Build a GeoJSON FeatureCollection with range band polygons.
    Bands should be ordered largest-to-smallest for proper layering.
    """
    features = []
    ...
    for band in bands:
        ...
        features.append({
            ...
            "properties": {
                "band": band["label"],
                "range_km": round(band["range_km"], 1),
...
def main():
    ...
    # 4. Calculate ranges for each energy band
    band_results = []
    for band in ENERGY_BANDS:
        range_km = calculate_range_km(
            soc=soc,
            soh=soh,
            temp_c=temp_c,
            wind_ms=wind_ms,
            energy_fraction=band["fraction"],
        )
        band_results.append({
            **band,
            "range_km": range_km,
    ...
    # 5. Fetch Valhalla isodistance polygons (single batch request)
    distances_km = []
    valid_indices = []
    for i, band in enumerate(band_results):
        if band["range_km"] >= 1.0:
            ...
            log.warning(f"  Band {band['label']}: range too small ({band['range_km']:.1f}km), skipping")
            band["geometry"] = None
    ...
    if distances_km:
        ...
        log.warning("No valid ranges to request from Valhalla")

    # 6. Build and write GeoJSON
    geojson = build_geojson(lat, lon, band_results)
    with open(GEOJSON_PATH, "w") as f:
        json.dump(geojson, f)
    log.info(f"GeoJSON written: {GEOJSON_PATH}")

    # 7. Build and write metadata
    ...
    metadata = {
        ...
        "vehicle": {
            "soc_pct": soc,
            "soh_pct": soh,
            "oem_range_km": vehicle["oem_range_km"],
        ...
        "range": {
            "intelligent_range_km": round(max_range, 1),
            "intelligent_range_miles": round(max_range * 0.621371, 1),
            "bands": [
                {
                    "label": b["label"],
                    "range_km": round(b["range_km"], 1),
                    "range_miles": round(b["range_km"] * 0.621371, 1),
                    "color": b["color"],
                    "has_polygon": b.get("geometry") is not None,
                }
                for b in band_results
            ],
        },
        "calculation": {
            "method": "valhalla_isodistance_v3.0",
    ...
    # Print summary for HA shell_command output
    print(json.dumps({
        "status": "ok",
        "range_km": round(max_range, 1),
        "range_miles": round(max_range * 0.621371, 1),
        "soc": soc,
